...
public class DrillRelMdSelectivity extends RelMdSelectivity {
    ...
    for (RexNode pred : RelOptUtil.conjunctions(predicate)) {
      ...
      for (RexNode orPred : RelOptUtil.disjunctions(pred)) {
        ...
        if (orPred.isA(SqlKind.EQUALS)) {
          ...
          orSel +=  Math.min(computeEqualsSelectivity(table, orPred, fieldNames) + 0.05,
          ...
          if (orPred instanceof RexCall) {
            ...
            if (childOp.isA(SqlKind.LIKE)) {
              orSel += 1.0 - Math.min(computeEqualsSelectivity(table, childOp, fieldNames) + 0.05,
  ...
  private double computeNullSelectivity(DrillTable table, RexNode orPred, List<String> fieldNames) {
    if (orPred instanceof RexCall) {
      int colIdx = -1;
      RexInputRef op = findRexInputRef(orPred);
      if (op != null) {
      ...
      if (colIdx != -1 && colIdx < fieldNames.size()) {
        ...
        if (table.getStatsTable() != null
          ...
          return Math.max(1.0/table.getStatsTable().getRowCount(),
              1.0 - (table.getStatsTable().getNNRowCount(col) / table.getStatsTable().getRowCount()));
        }
      } else {
        if (logger.isDebugEnabled()) {
          logger.warn(String.format("No input reference $[%s] found for predicate [%s]",
